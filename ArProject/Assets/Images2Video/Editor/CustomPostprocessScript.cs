...
public class CustomPostprocessScript {
	...
	public static void OnPostprocessBuild(BuildTarget target, string pathToBuildProject) {
		...
		PBXModifierForAssetsLib modForAssetsLib = new PBXModifierForAssetsLib();
		var lines = modForAssetsLib.applyToAssetLibrary (pbxproj);
		File.WriteAllLines (pbxproj, lines);

        PBXModifierForAccelerateLib modForAccelerateLib = new PBXModifierForAccelerateLib();
		
		lines = modForAccelerateLib.applyToAccelerateLibrary (pbxproj);
		File.WriteAllLines (pbxproj, lines);
	...
	private static void ProcessInfoPList(string path)
	{
		try {
			...
			if (dict != null) {
				...
				PListItem microphoneUsageDesc = GetPlistItem(dict, "NSMicrophoneUsageDescription");
				if (microphoneUsageDesc == null)
				{
					...
					str.InnerText = "Images2Video requires access to the microphone";
                ...
                //If iOS >= 11
                PListItem photoLibraryAddUsageDesc = GetPlistItem(dict, "NSPhotoLibraryAddUsageDescription");
				if (photoLibraryAddUsageDesc == null)
				{
					XmlElement key = xmlDocument.CreateElement("key");
					key.InnerText = "NSPhotoLibraryAddUsageDescription";

					XmlElement str = xmlDocument.CreateElement("string");
					str.InnerText = "Images2Video requires access to the photo library";
